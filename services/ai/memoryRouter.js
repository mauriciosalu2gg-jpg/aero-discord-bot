 ...
 * @param {'topic'|'summary'|'profile'} task - Tipo de tarea para seleccionar modelo.
 * @param {Array} messages - Mensajes en formato OpenAI [{role, content}].
 * @param {number} [temperature=0.3] - Temperatura de generación.
...
export async function askMemoryEngine(task, messages, temperature = 0.3) {
  ...
  if (providers.length === 0) {
    throw new Error('[MemoryEngine] No hay proveedores de memoria configurados (MEMORY_GROQ_KEY_1, MEMORY_GEMINI_KEY, etc).');
  }
...
export function isMemoryEngineAvailable() {
  ...
  return !!(cfg.groqKey1 || cfg.groqKey2 || cfg.geminiKey || cfg.ollamaUrl);
}
