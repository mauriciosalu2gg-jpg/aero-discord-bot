...
export async function handleEstado(interaction) {
  ...
  const configuredProviders = secrets.getAvailableProviders().map(p => p.name);
  const healthy = snapshots
    .filter(s => s.status === 'Healthy' && (!active || s.name !== active.name) && configuredProviders.includes(s.name))
