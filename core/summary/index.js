...
export async function summarizeMemoryHistory(memoryData) {
  ...
  const prompt = `Analiza la siguiente conversacion y extrae una lista concisa de DATOS IMPORTANTES o HECHOS sobre el usuario (preferencias, contexto, detalles relevantes). No resumas hola/chau.
Si ya existen hechos previos, combínalos inteligentemente.
Hechos Previos:
...
Devuelve SOLAMENTE una lista de viñetas con los hechos finales actualizados.`;
  ...
  try {
    const aiResponse = await askAI([{ role: 'user', content: prompt }], 0, {
      systemExtra: 'Eres un sistema de compresion de memoria. Devuelve solo viñetas (- dato).',
      intent: 'summary'
    });
